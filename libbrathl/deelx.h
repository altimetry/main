...
public:
	...
	void  Append(const ELT * pcsz, long length, int eol = 0);
...
template <class ELT> void CBufferT <ELT> :: Append(const ELT * pcsz, long length, int eol)
{
...
public:
	...
	CHART * Replace(const CHART * tstring, long string_length, const CHART * replaceto, long to_length, long & result_length, int start = -1, int ntimes = -1, MatchResult * result = 0, CContext * pContext = 0) const;
...
template <class CHART> CHART * CRegexpT <CHART> :: Replace(const CHART * tstring, long string_length, const CHART * replaceto, long to_length, long & result_length, int start, int ntimes, MatchResult * remote_result, CContext * oContext) const
{
	...
	if(rightleft)
	{
		...
		{
			result_string.Append(buffer[i], (long)buffer[i+1]);
		}
	}
	else
	{
