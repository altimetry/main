...
namespace brathl
{
	...
	{
		mLabel = "Topex/Poseidon product";	//BUT see GetLabel!
	...
	//virtual
	const std::string& CProductTopex::GetLabel() const
	{
		if ( m_fileList.m_productType.compare( CProductTopex::m_PASS_FILE ) == 0 )
		{
			const_cast<std::string&>( mLabel ) = "Topex/Poseidon GDR-M product";			//FIXME
		}
		else if ( m_fileList.m_productType.compare( CProductTopex::m_XNG_FILE ) == 0 )
		{
			const_cast<std::string&>( mLabel ) = "Topex/Poseidon crossover point product";	//FIXME
		}
	...
	//----------------------------------------
	CProductTopex::CProductTopex( const CStringList &fileNameList, bool check_only_first_file )
