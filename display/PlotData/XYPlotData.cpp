...
//----------------------------------------
void CXYPlotProperty::SetName(const std::string& value)
{
  m_name = value;
...
CXYPlotData::CXYPlotData(const std::string& name, CXYPlotProperty* plotProperty)
{
    UNUSED( name );
...
void CXYPlotData::SetRawData( CPlotArray* x, CPlotArray* y )
{
	...
	{
        std::string msg =
                "ERROR in CXYPlotData::SetRawData : Number of frames in PlotArray must be the same for all data - "
                "nb frames for x : "
...
//#if !defined BRAT_V3
		mQwtPlotData.SetFrameData( framey, newX, xDim, newY, yDim );
//#else
