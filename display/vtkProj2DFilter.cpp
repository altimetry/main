...
void vtkProj2DFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  switch (this->Projection)
    ...
    case VTK_PROJ2D_MERCATOR:
      ...
      break;
    case VTK_PROJ2D_TMERCATOR:
      os << "Transverse Mercator" << endl;
      break;
    case VTK_PROJ2D_ORTHO:
      os << "Orthographic" << endl;
      break;
    case VTK_PROJ2D_NEAR_SIGHTED:
      os << "Near-Sighted Perspective" << endl;
      break;
     case VTK_PROJ2D_STEREOGRAPHIC:
      os << "Stereographic" << endl;
      break;
    default:
  ...
  for (id = 0; id < numPoints; ++id)
    {
        ...
        if ( vectors )
        {
            ...
            if ( tuple )
            {
                ...
                // this is where the original vector points in 2D untransformed space
               ...
               // L = P + tv   -> v = vector normal to plane,
  ...
  for (id = 0; id < numPoints; ++id)
    ...
    newPoints->SetPoint(id, npt);
     ...
     if ( vectors )
     {
        ...
        // The following code would re-scale the vector, but
  ...
  for (id = 0; id < numPoints; id++)
    ...
    newPoints->SetPoint(id, npt);
     ...
     if ( vectors )
     {
        ...
        // We don't want to re-scale the vector now..
    ...
    for (id = 0; id < numPoints; ++id)
  {
    ...
    // transform the vector data
     if ( vectors && isVisible(npt[0], npt[1]))
     {
        ...
        // we don't want to rescale the vector..
...
void vtkSimpleVectorProjFilter::PerformSimple2D()
{
  ...
  {
      vtkErrorMacro(<<"Point data does not contain proper vector data.");
      return;
  }
  ...
  for (id = 0; id < numPoints; ++id)
    ...
    // convert the vector direction as well
    while (vdata[0] >= 180)
      {
      vdata[0] -= 360;
      }
    ...
    //return to vector, and not absolute point
