...
CPlotField* CPlotBase::GetPlotField(int32_t index)
{
  ...
  {
    string msg = CTools::Format("CPlotBase::GetPlotField - invalid index %d"
                                "- Valid range is [0, %d]",
                                index, nCount);
    CException e(msg,
                 BRATHL_LOGIC_ERROR);
...
//----------------------------------------
CPlotField* CPlotBase::FindPlotField(const wxString& fieldName, bool* withContour /* = NULL*/, bool* withSolidColor /* = NULL*/)
{
...
//----------------------------------------
void CPlotBase::GetVar(const string& varName, CInternalFiles* file,
                     ExpressionValueDimensions* dimVal,
                     CExpressionValue* var)
{
  if (file->VarExists(varName) == false)
  {
    string msg = CTools::Format("CPlotBase::GetAxis -  unknown variable name: '%s'",
                                varName.c_str());
  ...
  if (dimVal != NULL)
  {
    ...
    {
      string msg = CTools::Format("CPlotBase::GetAxis - '%s' axis -> number of dimensions must be 1 or 2 - Found : %ld",
...
{
  GetVar((const char *)(m_forcedVarXName), file, dimVal, varX);
}
//----------------------------------------
...
{
  GetVar((const char *)(m_forcedVarYName), file, dimVal, varY);
}
