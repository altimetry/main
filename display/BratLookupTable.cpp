...
const wxString ENTRY_CURVE = "ColorTable/Curve";
//const wxString ENTRY_STD = "ColorTable/Std";
const wxString GROUP_COLOR = "ColorDef";

#define brathlFmtEntryColorMacro(entry, index) \
  wxString::Format(GROUP_COLOR + "%d/" + entry, index)

const string CURVE_LINEAR = "Linear";
const string CURVE_SQRT = "SQRT";
const string CURVE_COSINUS = "Cosinus";
...
//----------------------------------------
CBratLookupTable::CBratLookupTable(CBratLookupTable& lut)
{
  ...
  if (method == NULL)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::CBratLookupTable(CBratLookupTable& lut) : unknown methodname '%s'",
                                 methodName.c_str());
...
//----------------------------------------
CBratLookupTable& CBratLookupTable::operator=(CBratLookupTable& lut)
{
...
void CBratLookupTable::Init()
{
  ...
  m_nameToMethod.Insert((const char *)(m_defaultColorTable),	BRATHL_CALLBACK(CallbackVoid, Aerosol));
  ...
  {
    m_colorTableList.Add((it->first).c_str());
  }
...
//----------------------------------------
string CBratLookupTable::MethodToLabeledMethod(const string& method)
{
  CObMap::iterator it;
  string methodTmp = CTools::StringToLower(method);

  for (it = m_nameToMethod.begin() ; it != m_nameToMethod.end() ; it++)
  {
    string lowerMethod = CTools::StringToLower(it->first);
...
//----------------------------------------
string CBratLookupTable::CurveToLabeledCurve(const string& curve)
{
  CStringArray::iterator it;
  string curveTmp = CTools::StringToLower(curve);

  for (it = m_curveNames.begin() ; it != m_curveNames.end() ; it++)
  {
    string lowerCurve = CTools::StringToLower(*it);
...
//----------------------------------------
bool CBratLookupTable::IsValidCurve(const string& curve)
{
  CStringArray::iterator it;
  for (it = m_curveNames.begin() ; it != m_curveNames.end() ; it++)
  {
...
//----------------------------------------
bool CBratLookupTable::IsValidMethod(const string& methodName)
{
  CallbackVoid* method = static_cast< CallbackVoid* >(m_nameToMethod[methodName]);
  if (method == NULL)
  {
...
//----------------------------------------
void CBratLookupTable::ExecMethod(const wxString& methodName)
{
  ExecMethod(string(methodName));
}

//----------------------------------------
void CBratLookupTable::ExecMethod(const string& methodName)
{
  ...
  if (method == NULL)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::ExecMethod : unknown methodname or not a CallbackVoid :'%s'",
                                 methodName.c_str());
    CException e(msg, BRATHL_LOGIC_ERROR);
  ...
  {
    string msg = CTools::Format("BRAT ERROR in CBratLookupTable::ExecMethod : exception while executing methodname :'%s'"
                                "\nNative Error : %s",
                                 methodName.c_str(),
                                 e.what());
  ...
  {
    string msg = CTools::Format("BRAT RUNTIME ERROR in CBratLookupTable::ExecMethod : exception while executing methodname :'%s'"
                                "\nNative Error : %s",
                                 methodName.c_str(),
                                 e.what());
    CException e2(msg, BRATHL_LOGIC_ERROR);
  ...
  {
    string msg = CTools::Format("BRAT FATAL ERROR in CBratLookupTable::ExecMethod : exception while executing methodname :'%s'",
                                 methodName.c_str());
    CException e2(msg, BRATHL_LOGIC_ERROR);
    CTrace::Tracer("%s",e2.what());
    Dump(*CTrace::GetDumpContext());
...
//----------------------------------------
void CBratLookupTable::ExecCurveMethod(const string& curve)
{
  string methodName = "SetCurve" + curve;
...
void CBratLookupTable::FacetsCorrection()
{
  ...
  if (m_cust.size() < 2)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::FacetsCorrection() - size of custom color array is less 2 : %ld",
                                (long)m_cust.size());
    CException e(msg, BRATHL_LOGIC_ERROR);
    throw(e);
  }
...
void CBratLookupTable::Gradient()
{
  ...
  if (m_grad.size() != 2)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::Gradient() - size of color array (must contains min/max) not equal 2 : %ld",
                                (long)m_grad.size());
    CException e(msg, BRATHL_LOGIC_ERROR);
    throw(e);
  }
...
void CBratLookupTable::Custom()
{
  ...
  if (m_cust.size() < 2)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::Custom() - size of custom color array is less 2 : %ld",
                                (long)m_cust.size());
    CException e(msg, BRATHL_LOGIC_ERROR);
    throw(e);
  }
  ...
  if (maxXValue < 0)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::Custom() - max of X value is negative value : %d",
                                  maxXValue);
    CException e(msg, BRATHL_LOGIC_ERROR);
    throw(e);
  }
...
//----------------------------------------
void CBratLookupTable::SaveToFile(const wxString& fileName)
{
  bool bOk = true;
  m_fileName = fileName;
  try
...
void CBratLookupTable::SaveCustToFile(wxFileConfig& file)
{
  ...
  if (m_cust.size() < 2)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::SaveCustToFile() - size of custom color array is less 2 : %ld",
                                (long)m_cust.size());
    CException e(msg, BRATHL_LOGIC_ERROR);
    throw(e);
  }
...
void CBratLookupTable::SaveGradToFile(wxFileConfig& file)
{
  ...
  if (m_grad.size() != 2)
  {
    string msg = CTools::Format("ERROR in CBratLookupTable::SaveGradToFile() - size of color array (must contains min/max) not equal 2 : %ld",
                                (long)m_grad.size());
    CException e(msg, BRATHL_LOGIC_ERROR);
    throw(e);
  }
...
//----------------------------------------
void CBratLookupTable::HandleLoadError(bool bOk, const wxString& entry)
{
  if (bOk == false)
  {
    CException e(CTools::Format("ERROR in CBratLookupTable::LoadFromFile - Can't read '%s' from file %s",
                                (const char *)(entry), (const char *)(m_fileName)),
                 BRATHL_LOGIC_ERROR);
    throw(e);
...
//----------------------------------------
void CBratLookupTable::HandleLoadColorError(bool bOk, const wxString& entry, double& color)
{
...
//----------------------------------------
bool CBratLookupTable::LoadFromFile(const wxString& fileName)
{
  bool bOk = true;
...
bool CBratLookupTable::LoadGradFromFile(wxFileConfig& file)
{
  ...
  while (bOk)
  {
    ...
    if (isColorDef)
    {
      ...
      {
        wxString msg = wxString::Format("Invalid value %d for '%s/Value' \n Color table will not be loaded",
                                        xValue,
                                        group.c_str());
        ::wxMessageBox(msg,
                       "Warning",
...
//----------------------------------------
int32_t CBratLookupTable::InsertCustomColor(CCustomColor *color)
{
  if (m_cust.size() <= 0)
  {
    m_cust.Insert(color);
  ...
  if (duplicate)
  {
    ::wxMessageBox(wxString::Format("There are duplicate color definitions (value %d) in file '%s'\n Color defintion will be ignored",
                                    color->GetXValue(), m_fileName.c_str()),
               "Warning",
                wxOK | wxCENTRE | wxICON_EXCLAMATION);
    return -1;
  }
...
//----------------------------------------
void CBratLookupTable::Dump(ostream& fOut /* = cerr */)
{
  ...
  fOut << "==> Dump a CBratLookupTable Object at "<< this << endl;


  fOut << "m_currentFunction = " << m_currentFunction << endl;
  fOut << "m_curve " << m_curve << endl;
//  fOut << "m_std = " << m_std << endl;
  fOut << "m_vtkLookupTable at " << m_vtkLookupTable << endl;

  m_nameToMethod.Dump(fOut);


  fOut << "==> END Dump a CBratLookupTable Object at "<< this << endl;
