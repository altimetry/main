...
//----------------------------------------
void vtkObList::Dump(ostream& fOut /* = cerr */)
{
  ...
  if (CTrace::IsTrace() == false)
   ...
   fOut << "==> Dump a vtkObList Object at " << this << " with " <<  size() << " elements" << endl;

   for ( it = this->begin( ); it != this->end( ); it++ )
   {
     fOut << "vtkObList[" << i << "] at " << (*it) << endl;  
     //(*it)->Dump(fOut);
     i++;
   }

   fOut << "==> END Dump a vtkObList Object at " << this << " with " <<  size() << " elements" << endl;
...
//----------------------------------------
void vtkObArray::Dump(ostream& fOut /* = cerr */)
{
  ...
  if (CTrace::IsTrace() == false)
   ...
   fOut << "==> Dump a vtkObArray Object ast "<< this << " with " <<  size() << " elements" << endl;

   for ( it = this->begin( ); it != this->end( ); it++ )
   {
     fOut << "vtkObArray[" << i << "] at " << (*it) << endl;  
     //(*it)->Dump(fOut);
     i++;
   }

   fOut << "==> END Dump a vtkObArray Object at "<< this << " with " <<  size() << " elements" << endl;
...
vtkObject* vtkObMap::Insert(const string& key, vtkObject* ob, bool withExcept /* = true */)
{
  

  pair <vtkObMap::iterator,bool> pairInsert;
...
vtkObject* vtkObMap::Exists(const string& key)
{
...
bool vtkObMap::Erase(const string& key)
{
...
vtkObject* vtkObMap::operator[](const string& key)
{
...
void vtkObMap::Dump(ostream& fOut /* = cerr */) 
{
   ...
   fOut << "==> Dump a vtkObMap Object at "<< this << " with " <<  size() << " elements" << endl;
   ...
   {
      vtkObject *ob = it->second;
      fOut << "vtkObMap Key is = " << (*it).first << endl;
      fOut << "vtkObMap Value at " << ob << endl;
      //ob->Dump(fOut);
   }

   fOut << "==> END Dump a vtkObMap Object at "<< this << " with " <<  size() << " elements" << endl;
