...
bool CMapProjection::ValidName(const string& name)
{
...
//----------------------------------------
string CMapProjection::NameToLabeledName(const string& name)
{
  CMapProjection::iterator it;
  string nameTmp = CTools::StringToLower(name);

  for (it = begin() ; it != end() ; it++)
  {
    string lowerName = CTools::StringToLower(it->first);
...
string CMapProjection::IdToName(uint32_t id)
{
  CMapProjection::iterator it;
...
//----------------------------------------
uint32_t CMapProjection::NameToId(const string& name)
{
  return Exists(name);
}
...
uint32_t CMapProjection::Exists(const string& key)
{
...
uint32_t CMapProjection::operator[](const string& key)
{
   return Exists(key);
}
