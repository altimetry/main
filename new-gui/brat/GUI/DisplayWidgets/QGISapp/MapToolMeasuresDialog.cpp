...
CMeasuresToolDialog::CMeasuresToolDialog( CMeasuresTool* tool, const QgsCoordinateReferenceSystem &crs, Qt::WindowFlags f )
...
void CMeasuresToolDialog::updateSettings( const QgsCoordinateReferenceSystem &crs )
{
	QSettings settings;
	...
	//mDa.setEllipsoid( QgsProject::instance()->readEntry( "Measure", "/Ellipsoid", GEO_NONE ) );	//	TODO
	qDebug() << QgsProject::instance()->readEntry( "Measure", "/Ellipsoid", GEO_NONE );	
	mDa.setEllipsoid( crs.ellipsoidAcronym() );														//	mDa.setEllipsoid( "WGS84" );
...
void CMeasuresToolDialog::unitsChanged( int index )
{
	if ( mMeasureArea )
...
void CMeasuresToolDialog::restart()
{
	mTool->restart();
...
void CMeasuresToolDialog::mouseMove( const QgsPoint &point )
{
...
void CMeasuresToolDialog::addPoint( const QgsPoint &p )
{
	Q_UNUSED( p );
...
void CMeasuresToolDialog::removeLastPoint()
{
	int numPoints = mTool->points().size();
	if ( mMeasureArea )
	{
...
void CMeasuresToolDialog::closeEvent( QCloseEvent *e )
{
	reject();
	e->accept();
}

void CMeasuresToolDialog::restorePosition()
{
	QSettings settings;
...
void CMeasuresToolDialog::saveWindowLocation()
{
	QSettings settings;
...
QString CMeasuresToolDialog::formatDistance( double distance, bool convertUnits ) const
{
	QSettings settings;
...
QString CMeasuresToolDialog::formatArea( double area, bool convertUnits ) const
{
	if ( convertUnits )
...
void CMeasuresToolDialog::updateUi()
{
...
void CMeasuresToolDialog::repopulateComboBoxUnits( bool isArea )
{
	mUnitsCombo->clear();
	if ( isArea )
	{
...
double CMeasuresToolDialog::convertLength( double length, QGis::UnitType toUnit ) const
{
	return mDa.convertLengthMeasurement( length, toUnit );
}

double CMeasuresToolDialog::convertArea( double area, QgsUnitTypes::AreaUnit toUnit ) const
{
	return mDa.convertAreaMeasurement( area, toUnit );
}


void CMeasuresToolDialog::reject()
{
	saveWindowLocation();
	restart();
	QDialog::close();
...
#include "moc_MapToolMeasuresDialog.cpp"
