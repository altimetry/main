...
CApplicationSettingsDlg::CApplicationSettingsDlg( CBratSettings &options, QWidget *parent ) : QDialog( parent ), mSettings( options )
{
    ...
    DataDirectory_lineEdit->setText( settings_paths->PortableBasePath().c_str() );
    ProjectsDirectory_lineEdit->setText( settings_paths->mWorkspacesDir.c_str() );

    AutoRelativePaths_checkBox->setChecked( settings_paths->UsePortablePaths() );
...
{
    QString dir = BrowseDirectory( this, "Select Default Data Directory", DataDirectory_lineEdit->text() );
    if ( !dir.isEmpty() )
...
{
    QString dir = BrowseDirectory( this, "Select Default Workspaces Directory", ProjectsDirectory_lineEdit->text() );
    if ( !dir.isEmpty() )
