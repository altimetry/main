...
//#include <osgEarthUtil/SkyNode>	femm
#include <osgEarthUtil/Sky>			//femm
...
//constructor
GlobePlugin::GlobePlugin( QgisInterface* theQgisInterface, QgsMapCanvas *theCanvas )
	: mCanvas( theCanvas )
	, mOsgViewer( 0 )
	, mRootNode( 0 )
	, mMapNode( 0 )
	...
	, mSelectedElevation( 0. )
{
	...
	//femm         mSettingsDialog = new QgsGlobePluginDialog( theQgisInterface->mainWindow(), this, QgisGui::ModalDialogFlags );
	mSettingsDialog = new QgsGlobePluginDialog( mCanvas->nativeParentWidget(), this, QgisGui::ModalDialogFlags );

	//femm moved from initGUI
...
osgViewer::Viewer* GlobePlugin::run( const std::string &imgDir )
{
	...
	setSkyParameters( settings.value( "/Plugin-Globe/skyEnabled", true ).toBool()
		, settings.value( "/Plugin-Globe/skyDateTime", QDateTime() ).toDateTime()
		, settings.value( "/Plugin-Globe/skyAutoAmbient", true ).toBool() );
...
{
	QgsMapCanvas* mapCanvas = mCanvas;	// mQGisIface->mapCanvas();
...
//femm    mTileSource = new QgsOsgEarthTileSource( mQGisIface );
  ...
  if ( mRootNode )
  {
    if ( enabled )
    {
      ...
      if ( !mSkyNode.get() )
        ...
        //mSkyNode = new SkyNode( mMapNode->getMap() );	femm
...
#endif
      mSkyNode->setDateTime( osgEarth::DateTime( dateTime.date().year()
                             , dateTime.date().month()
                             , dateTime.date().day()
                             , dateTime.time().hour() + dateTime.time().minute() / 60.0 ) );
      //mSkyNode->setDateTime( dateTime.date().year()	femm
      //                       , dateTime.date().month()
      //                       , dateTime.date().day()
      //                       , dateTime.time().hour() + dateTime.time().minute() / 60.0 );
