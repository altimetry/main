...
QgsAppLayerTreeViewMenuProvider::QgsAppLayerTreeViewMenuProvider( QgsLayerTreeView* view, QbrtMapEditor *editor )
	: mView( view )
	, mEditor( editor )
{}
...
QMenu* QgsAppLayerTreeViewMenuProvider::createContextMenu()
{
	...
	if ( !idx.isValid() )
	{
		...
		menu->addAction( QIcon( ":/images/actions/mActionExpandTree.png" ), tr( "&Expand All" ), mView, SLOT( expandAll() ) );
		menu->addAction( QIcon( ":/images/actions/mActionCollapseTree.png" ), tr( "&Collapse All" ), mView, SLOT( collapseAll() ) );
	...
	else if ( QgsLayerTreeNode* node = mView->layerTreeModel()->index2node( idx ) )
	{
		...
		{
			menu->addAction( actions->actionZoomToGroup( mEditor->canvas(), menu ) );

			menu->addAction( QIcon( ":/images/actions/mActionRemoveLayer.svg" ), tr( "&Remove" ), mEditor, SLOT( removeLayer() ) );
